class AddStudentPage {
    constructor() {
        this.registerHeading = '.register-heading'
    ...
    async confirmStudentPage() {
        const addStudentPageMessage = await page.locator(this.registerHeading).innerText()
        expect(addStudentPageMessage).toBe('Register New Student')
    }

    async fillStudentDetails(information) {
    ...
    async displaySuccessMessage(message) {
        const actualSuccessMessage = await page.locator(this.successMessage).innerText()
        expect(actualSuccessMessage).toBe(`"${message}"`);
    }
}
